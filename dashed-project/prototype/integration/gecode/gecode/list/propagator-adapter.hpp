...
inline ExecStatus reified_equal(Space& home, Propagator& propagator,
  ...
  Domain dx = x.domain();
  Domain dy = y.domain();
  Backend::BoolDomain db = read_bool(truth);
  const auto result = Backend::propagate_reified_equal(dx, dy, db);
  if (result.failed())
    return ES_FAILED;
  if (!commit_list(home, x, std::move(dx), result.left) ||
      !commit_list(home, y, std::move(dy), result.right) ||
...
inline ExecStatus reified_not_equal(Space& home, Propagator& propagator,
  ...
  Domain dy = y.domain();
  Backend::BoolDomain db = read_bool(truth);
  const auto result = Backend::propagate_reified_not_equal(dx, dy, db);
  if (result.failed())
    return ES_FAILED;
  if (!commit_list(home, x, std::move(dx), result.left) ||
      !commit_list(home, y, std::move(dy), result.right) ||
