namespace Gecode { namespace String { namespace Branch {
    ...
    forceinline
    DimDeg_LenBlock_Min_LLLM::DimDeg_LenBlock_Min_LLLM(Home home, 
                                              ViewArray<String::StringView>& x0)
    : StringBrancher(home, x0) {};

    forceinline 
    DimDeg_LenBlock_Min_LLLM::DimDeg_LenBlock_Min_LLLM(Home home, 
                              DimDeg_LenBlock_Min_LLLM& b)
    : StringBrancher(home, b, b.start) {
      x.update(home, b.x);
    }

    forceinline Actor*
    DimDeg_LenBlock_Min_LLLM::copy(Space& home) {
      return new (home) DimDeg_LenBlock_Min_LLLM(home, *this);
    }

    forceinline void
    DimDeg_LenBlock_Min_LLLM::post(Home home, ViewArray<String::StringView>& x) {
      (void) new (home) DimDeg_LenBlock_Min_LLLM(home, x);
    }

    /* Pick the variable x with minimal ratio dimension/degree. Tie-breaking with:
       - min. ub(|x|)-lb(|x|), tie-breaking with:
       - min. dimension of the leftmost unfixed block bx of x
       - min. ub(bx)-lb(bx)
       - min. lb(|x|)
    */
    forceinline Choice*
    DimDeg_LenBlock_Min_LLLM::choice(Space& home) {
//      std::cerr << "\nVar. choice\n";      
      if (_FIRST) {
...
//        std::cerr<<"Must chars: "<<CharSet(home,s)<<"\n";        
      ...
      double g = d / vx.degree();
      int pos = start;
//      std::cerr<<x[start]<<" (pos. "<<start<<", dim. "<<d<<")\n";
      for (int i = start + 1; i < x.size(); ++i) {
        StringView& xi = x[i];
//        std::cerr<<i<<": "<<xi<<' '<<x.size()<<"\n";
        if (!xi.assigned()) {
          double gi = xi.degree();
          ...
          gi = di / gi; 
//          std::cerr<<i<<": "<<xi<<" (pos. "<<i<<", dim. "<<di<<")\n";
          if (gi < g) { g = gi, f = fi, d = di, l = li, m = mi, pos = i; continue; }
          else if (gi > g) continue;
