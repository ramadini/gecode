...
public:
  ...
  Gray(const StringOptions& so): Script(so) {
    ...
    CharSet s(*this, '0', '0'+M-1);
    StringVarArgs sva;
    StringVarArgs aux;
    IntVarArgs iva;
    sva << StringVar(*this, Block(*this, s, N, N));
    for (int i = 1; i < MAX; ++i) {
      sva << StringVar(*this, Block(*this, s, N, N));
      StringVar a1(*this, Block(*this, s, 1, 1));
      StringVar a2(*this, Block(*this, s, 1, 1));
      aux << a1 << a2;
      IntVar m(*this, 1, N);
      find(*this, sva[i-1], a1, m);
  ...
  print(std::ostream& os) const {
    for (int i = 0, j = MAX; i < MAX; ++i) {
      os << "x[" << i << "] = " << vec2str(string_vars[i].val());
      if (i > 0)
        os << " (replaced 1st occurrence of " << vec2str(string_vars[j++].val())
           << " with " << vec2str(string_vars[j++].val()) << ")";
      os << '\n';
    }
    os << "----------\n";
    S++;
