...
public:
  ...
  Benchmark(const StringOptions& so): Script(so) {
    ...
    StringVar expr(*this, 1, so.N);
    StringVar blank1(*this, NSIntSet(' '), 0, so.N);
    StringVar blank2(*this, NSIntSet(' '), 0, so.N);

    // Constraints.
    StringVarArgs s;
    s << pref   << expr << blank1 << StringVar(*this, "=")
      << blank2 << expr << suff;
    gconcat(*this, s, StringVar(*this, so.SQL));
    StringVarArgs sva;
    sva << s;
...
int main(int argc, char* argv[]) {
  int n = atoi(argv[1]);
  StringOptions opt((string("*** gsql ") + argv[1] + string(" ***")).c_str(), n);
  opt.solutions(1);
