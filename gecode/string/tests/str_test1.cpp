...
public:
  ...
  void test01() {
    ...
    assert(eq(*this, x, y).propagate(*this, 0) == ES_SUBSUMED_);
    ...
    assert(nq(*this, x, z).propagate(*this, 0) == ES_SUBSUMED_);
    std::cerr << z << std::endl;
    assert (!z.assigned());
    ...
    BoolVar b0(*this, 0, 1);
    assert(eqv(*this, x, y, b0).propagate(*this, 0) == ES_SUBSUMED_);
    assert(b0.val() == 1);
    BoolVar b1(*this, 0, 1);
    assert(imp(*this, y, z, b1).propagate(*this, 0) == ES_SUBSUMED_);
    assert(b1.val() == 0);
    BoolVar b2(*this, 0, 1);
    assert(pmi(*this, z, y, b2).propagate(*this, 0) == ES_SUBSUMED_);
  ...
  void test03() {
    ...
    assert(cat(*this, x, y, z).propagate(*this, 0) == ES_SUBSUMED_);
